...
public class SearchServiceManager implements SearchService {
    ...
    public List<SearchResultVO> search(String keyword, String tag, int page, int size) {
        switch (mode) {
            case "mysql":
                return mysqlSearchService.search(keyword, tag, page, size);
            case "es":
                return esSearchService.search(keyword, tag, page, size);
            case "auto":
                return searchWithFallback(keyword, tag, page, size);
            default:
                throw new BusinessException("无效搜索模式: " + mode);
        }
    }

    private List<SearchResultVO> searchWithFallback(String keyword, String tag, int page, int size) {
        try {
            List<SearchResultVO> results = esSearchService.search(keyword, tag, page, size);
